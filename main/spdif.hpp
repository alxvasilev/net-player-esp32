...
{
public:
    enum { kFifoSize = 1024, kFifoClearSize = 128 };
...
public:
     ...
     void disable() { mConf1Reg.rx_en = false; }
     void zeroFifo() {
...
{
public:
    enum { kMaxPulsesPerBatch = 490 };
protected:
    RmtRxChannel mChan0;
    RmtRxChannel* mActiveChannel = nullptr;
    bool mIsFirstBatch = true;
    GpioPin mInputPin;
    RmtRxChannel mChan1;
    uint16_t mPulseClockDuration = 0;
    uint16_t pcntCurrPulseCount();
    void pcntReset();
    void pcntInit();
public:
    RmtRx(gpio_num_t inputPin);
    RmtRxChannel* activeChanel() const { return mActiveChannel; }
    void start();
