...
bool AccessKeyFetcher::isExpired() {
  ...
  return mCtx.mTimeProvider.getSyncedTimestamp() > expiresAt;
}
...
void AccessKeyFetcher::updateAccessKey() {
  ...
  loginRequest.client_info.device_id.arg = &mCtx.mConfig.deviceId;

  loginRequest.login_method.stored_credential.username.funcs.encode =
      &bell::nanopb::encodeString;
  loginRequest.login_method.stored_credential.username.arg = &mCtx.mConfig.username;
  ...
  loginRequest.login_method.stored_credential.data.arg = &mCtx.mConfig.authData;
  ...
  do {
    ...
    auto responseBytes = httpPost<std::vector<uint8_t>>(
        "https://login5.spotify.com/v3/login", {{"Content-Type", "application/x-protobuf"}},
        (const char*)encodedRequest.data(), encodedRequest.size());
    ...
    if (loginResponse.which_response == LoginResponse_ok_tag) {
      ...
      this->expiresAt = mCtx.mTimeProvider.getSyncedTimestamp() + (expiresIn * 1000);
    }
    else {
